...
def lambda_handler(event, context):
    ...
    # requests forecasted load data for each YYYYMMDD date, then concats the results into a tall dataframe 
    ...
    with httpx.Client(headers=auth) as client:
        while attempts < 3:
            ...
            for date in date_range:
                try:
                    r = client.get(url = base_url+'/hourlysysload/day/'+date+'/.json', timeout=None)
...
def clean_data(json_list):
    ...
    df_list = [pd.json_normalize(json['HourlySystemLoads']['HourlySystemLoad'], sep='_') for json in json_list]
