...
module "prophet_forecast" {
  source = "terraform-aws-modules/lambda/aws"

  function_name = "${random_pet.prophet_forecast.id}-lambda-from-container-image"
  description   = "Creates forecasts with Prophet."
  ...
  # Container Image
  ##################
  image_uri     = module.docker_image_prophet_forecast.image_uri
  package_type  = "Image"
  architectures = ["x86_64"]
